...
app.get('/api/members/me', authenticateToken, async (req, res) => {
    try {
        ...
        const memberData = { id: user.id, fullName: user.name, email: user.email, phone: user.phone, idNumber: user.idNumber, monthlyContribution: user.monthlyContribution, monthlyTarget: user.monthlyTarget, yearlyTarget: user.yearlyTarget, createdAt: user.createdAt };
...
app.put('/api/members/me', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const userRes = await db.query('SELECT u.*, gm."groupId", gm.role FROM users u LEFT JOIN group_members gm ON u.id = gm."userId" WHERE u.id = $1', [req.user.id]);
        const user = userRes.rows[0];
        res.json({ member: { id: user.id, fullName: user.name, email: user.email, phone: user.phone, idNumber: user.idNumber, monthlyContribution: user.monthlyContribution, monthlyTarget: user.monthlyTarget, yearlyTarget: user.yearlyTarget, createdAt: user.createdAt } });
...
app.get('/api/admin/users', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT u.id, u.name, u.email, u."isSuperAdmin", gm."groupId", gm.role, g.name as "groupName" FROM users u LEFT JOIN group_members gm ON u.id = gm."userId" LEFT JOIN stokvel_groups g ON gm."groupId" = g.id`);
